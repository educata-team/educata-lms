...
interface Props{
  ...
  setInfoFromFirstPage(email: string, password: string): void;
}
  ...
  return (
    <Grid>
      <Paper elevation={10} style = { { padding :20,height: '60vh',width:380, margin:'20px auto', display: 'flex', flexDirection: 'column', justifyContent: 'space-around' } }>
        ...
        <TextField label="Password" placeholder="Repeat password" type="password"  required onChange={(event: React.ChangeEvent<HTMLInputElement>):void => {setRepeat(event.target.value);}}/>

        {/* eslint-disable-next-line no-console */}
        <Button type="submit" variant="contained" style={ { margin:'8px 0' } }  onClick={():void => {if (password === repeat){setInfoFromFirstPage(email, password); goToPage(2);}}}>Next</Button>
